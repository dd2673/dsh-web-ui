// Adapted from Lum1104/dsh-browser at 782c61d39cd71394c9901174b09f69cfd4a041ab. See LICENSE.dsh-browser.
/**
...
export function createRemoteHostApi(
  ...
  return new RemoteHostApi(gateway, connection, includeControl)
}
...
class RemoteHostApi implements BrowserHostApi {
  ...
  constructor(
    ...
    connection: HostConnectionLike,
    private readonly includeControl: boolean,
  ) {
  ...
  private async sessionHistory(call: HostRpcCall): Promise<HostRpcResult> {
    ...
    try {
      ...
      // Reading the selected device conversation authorizes this surface to render its next interaction.
      if (this.includeControl) this.extensionSessions.note(sessionId)
  ...
  /** Reopen the durable selection instead of displaying the Host default for every session. */
  private async sessionModels(call: HostRpcCall): Promise<HostRpcResult> {
    try {
      const catalog = await this.gateway.invoke({ namespace: 'session', method: 'modelCatalog', args: {}, signal: call.signal })
      if (!isRecord(catalog)) throw new TypeError('session/modelCatalog returned an invalid catalog')
      let current = catalog.default
      const sessionId = sessionIdOf(call.payload)
      if (sessionId !== undefined) {
        const snapshot = await oneShotSessionSnapshot(this.gateway, sessionId, call.signal, 1)
        const projections = snapshot.projections
        const values = isRecord(projections) ? projections.values : undefined
        const selection = isRecord(values) ? values.modelSelection : undefined
        if (isRecord(selection)) current = selection.next ?? selection.lastUsed ?? current
      }
      return { ok: true, value: { ...catalog, current } }
    } catch (error: unknown) { return { ok: false, error: this.failure(error) } }
  }
...
class EventGeneration {
  ...
  start(): void {
    ...
    if (this.includeControl) this.track(this.pumpControl())
  }
  ...
  async respond(rpcId: string, result: RespondResult, signal: AbortSignal): Promise<unknown> {
    ...
    let outcome = respondOutcome(result)
    if (pending.kind === 'approval' && result.ok) {
      const reply = isRecord(result.value) ? result.value : undefined
      if (reply?.sessionId !== pending.sessionId || reply.approvalId !== rpcId
        || !['allowed-once', 'rejected', 'cancelled'].includes(String(reply.outcome))) {
        return { accepted: false, reason: 'invalid-approval' }
      }
      outcome = { kind: 'result', value: reply.outcome }
    }
    pending.settled = true
    try {
      await this.sendResult(clientId, rpcId, outcome, AbortSignal.any([this.signal, signal]))
  ...
  /** Keep queue/projection baselines and updates in the original device wire shape. */
  private async pumpControl(): Promise<void> {
    try {
      const source = await this.gateway.wireStream.open('session/control', { args: {} }, this.signal)
      for await (const frame of source) {
        if (this.signal.aborted) break
        if (!isRecord(frame)) throw new TypeError('Invalid session/control frame')
        if (frame.type === 'baseline' && isRecord(frame.value)) {
          for (const [key, kind] of [['queues', 'queue'], ['jobs', 'jobs'], ['projections', 'projection']] as const) {
            const map = frame.value[key]
            if (!isRecord(map)) continue
            for (const [sessionId, value] of Object.entries(map)) {
              if (kind === 'projection' && isRecord(value) && isRecord(value.values)) {
                for (const [projectionKey, projection] of Object.entries(value.values)) this.pushControl({ type: kind, sessionId, key: projectionKey, value: projection, seq: value.asOfSeq })
              } else if (kind !== 'projection') this.pushControl({ type: kind, sessionId, [kind === 'queue' ? 'items' : 'jobs']: value })
            }
          }
        } else if (['queue', 'jobs', 'projection'].includes(String(frame.type))) this.pushControl(frame)
        else throw new TypeError('Unknown session/control frame')
      }
      if (!this.signal.aborted) throw new Error('session/control ended unexpectedly')
    } catch (error: unknown) { if (!this.signal.aborted) this.queue.fail(error) }
  }

  private pushControl(frame: Record<string, unknown>): void {
    const type = `session/${String(frame.type)}`
    this.queue.push({ rpcId: crypto.randomUUID(), method: type, payload: { ...frame, type } })
  }
  ...
  private async handleRemoteEvent(value: unknown): Promise<void> {
    ...
    if (value.type === 'emit') {
      if (this.includeControl && Array.isArray(value.args)) {
        const [sessionId, detail] = value.args
        const names: Record<string, string> = { 'api-session/status': 'session/status', 'api-session/activity': 'session/activity', 'api-session/error': 'session/error' }
        const type = typeof value.event === 'string' ? names[value.event] : undefined
        if (type !== undefined) this.queue.push({ rpcId: crypto.randomUUID(), method: type, payload: { type, sessionId, ...(type === 'session/status' ? { running: detail } : type === 'session/activity' ? { updatedAt: detail } : { error: detail }) } })
      }
      return
    }
    if (value.type === 'cancel' && typeof value.eventId === 'string') {
      ...
      this.queue.push({
        ...
        payload: {
          ...
          ...(pending.kind === 'approval' ? { approvalId: value.eventId } : { questionRpcId: value.eventId }),
        },
      })
      return
    }
    ...
    const approval = this.includeControl && value.event === 'approval/request' && typeof value.request.toolName === 'string'
    if (!approval && (value.event !== 'user-questions/request' || !Array.isArray(value.request.questions))) {
      const clientId = this.clientId
    ...
    this.pendingQuestions.set(value.eventId, { sessionId: value.agentId, settled: false, kind: approval ? 'approval' : 'question' })
    this.queue.push({
      rpcId: value.eventId,
      method: approval ? 'approval/requested' : 'question/requested',
      payload: approval ? { ...value.request, type: 'approval/requested', sessionId: value.agentId, approvalId: value.eventId } : {
...
function invokeTarget(call: HostRpcCall): InvokeTarget | { readonly error: HostRpcFailure } {
  ...
  switch (call.method) {
    ...
    case 'host.listDirectory':
      return { namespace: 'directoryPicker', method: 'list', args: { path: call.payload.path } }
    case 'skill.list':
      return { namespace: 'skills', method: 'list', args: { request: call.payload } }
    case 'agentPreset.list':
      return { namespace: 'agentPresets', method: 'list', args: {} }
    case 'agentPreset.select':
      return { namespace: 'agentPresets', method: 'select', args: { agentId: call.payload.sessionId, agentPreset: call.payload.agentPreset }, adapt: value => ({ agentPreset: value }) }
