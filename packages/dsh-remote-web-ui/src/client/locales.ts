...
export const zh = {
  ...
  'relayToken.title': 'Android 安全连接',
  'relayToken.hint': 'DeepSeek 端只需填写 Relay 服务地址；手机扫码后自动写入主机标识和一次性配对凭据。服务器仅保存 hash。',
  'relayToken.once': '用 Android App 扫码连接。二维码中的凭据首次兑换后立即作废；轮换后旧手机也会失效。',
  'relayToken.target': 'Relay：{relay} · 主机：{host}',
  'relayToken.expires': '扫码配对有效至 {time}',
  'relayToken.expired': '扫码配对已过期，请生成新二维码。已完成兑换的手机不受此时间影响。',
  'relayToken.none': '尚未生成手机连接二维码。',
  'relayToken.configured': '服务端已确认连接凭据，指纹：{fingerprint}',
  'relayToken.rotate': '生成或更换连接二维码',
  'relayToken.revoke': '撤销令牌',
  ...
  'settings.relayUrl': 'Relay 服务地址',
  'settings.relayUrlHint': '只需填写服务端分配的 WSS 地址；留空即停用公网中继。主机 ID 与服务凭据由部署内部管理。',
...
export const en = {
  ...
  'relayToken.title': 'Secure Android connection',
  'relayToken.hint': 'DeepSeek only needs the Relay service URL. Scanning fills the internal host identity and a single-use pairing credential; the server stores hashes only.',
  'relayToken.once': 'Scan with the Android app. The QR credential expires immediately after exchange; rotation also invalidates the old phone.',
  'relayToken.target': 'Relay: {relay} · Host: {host}',
  'relayToken.expires': 'Scan-to-pair expires at {time}',
  'relayToken.expired': 'Scan-to-pair has expired. Generate a new QR; already exchanged phones remain valid.',
  'relayToken.none': 'No phone connection QR has been generated.',
  'relayToken.configured': 'Relay-confirmed credential, fingerprint: {fingerprint}',
  'relayToken.rotate': 'Generate or rotate connection QR',
  'relayToken.revoke': 'Revoke token',
  ...
  'settings.relayUrl': 'Relay service URL',
  'settings.relayUrlHint': 'Enter only the assigned WSS service URL; blank disables the public relay. Host ID and service credentials stay deployment-internal.',
